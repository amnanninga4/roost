...
**Schema versions.** `meta.schemaVersion` (2). `db.js` `migrate` runs on open, before seeding; v2 rebuilt `chores` and `handoffs` to widen the cadence CHECK and add the two chores columns. A fresh database gets the current shape and only records the version.
