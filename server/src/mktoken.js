...
// ROOST_DB picks the database (default /var/lib/roost/roost.db) so the first mint can stamp activeFrom.
import { readFileSync, writeFileSync, existsSync } from "node:fs";
import { randomBytes } from "node:crypto";
import { PEOPLE, openDb, ensureActiveFrom } from "./db.js";
...
// First mint (or first pair) stamps household start if unset; later mints leave it alone.
const db = openDb(dbPath);
const activeFrom = ensureActiveFrom(db, new Date());
db.close();
