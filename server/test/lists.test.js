...
test("lists start empty and need auth", async () => {
  ...
  const sync = await call("GET", "/sync?choresVersion=3", { token: ANNE });
...
test("meals: create, patch fields, nextUp is exclusive with a seq per cleared row, delete", async () => {
  ...
  const delta = await call("GET", `/sync?cursor=${seqBefore}&choresVersion=3`, { token: ANNE });
...
test("/sync carries all five list arrays, cursor is the max seq across tables, same-tick write after a sync is delivered", async () => {
  const full = await call("GET", "/sync?choresVersion=3", { token: ANNE });
  ...
  const idle = await call("GET", `/sync?cursor=${full.body.cursor}&choresVersion=3`, { token: ANNE });
  ...
  const delta = await call("GET", `/sync?cursor=${cursor}&choresVersion=3`, { token: ANNE });
  ...
  const mixed = await call("GET", `/sync?cursor=${c2}&choresVersion=3`, { token: ANNE });
  assert.deepEqual(
    {
