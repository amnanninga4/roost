...
final class DueWindowTests: XCTestCase {
    ...
    func day(_ month: Int, _ day: Int,
             year: Int = 2026) -> Date
    {
        cal.date(year: year, month: month, day: day, hour: 9)
    }
    ...
    func testGarbageIsFridaySaturdayThenLate() {
        for d in 14 ... 17 {
            XCTAssertNil(overdue(garbage, day(9, d)), "Sep \(d) is before the window")
        }
        ...
        for d in 21 ... 24 {
            XCTAssertNil(overdue(garbage, day(9, d), sunday))
        }
    ...
    func testCanIsSundayOnly() {
        for d in 14 ... 19 {
            XCTAssertNil(overdue(can, day(9, d)))
        }
    ...
    func testLitterChangeIsTheWeekEndingOnThe25th() {
        for d in 14 ... 18 {
            XCTAssertNil(overdue(litter, day(9, d)))
        }
        ...
        for d in 1 ... 18 {
            XCTAssertNil(overdue(litter, day(10, d), doneEarly))
        }
    ...
    func testBimonthlyAndQuarterlyUseThePeriodsLastMonth() {
        ...
        XCTAssertTrue(scheduler.dueItems(for: pantry, on: day(9, 29), completions: [done(pantry, .anne, day(9, 23))])
            .isEmpty)
    }
    ...
    func testWindowClosedBeforeTheHouseholdStartedWasNeverOwed() {
        ...
        for d in 14 ... 28 {
            XCTAssertNil(overdue(cushions, day(9, d)), "Sep \(d)")
        }
        ...
        let sundayStart = Scheduler(
            chores: [garbage],
            activeFrom: cal.date(year: 2026, month: 9, day: 13, hour: 0),
            calendar: cal
        )
    ...
    func testAnEarlyWindowBelongsToItsPeriodForDatesAndCompletions() {
        ...
        for d in 1 ... 29 {
            XCTAssertNil(overdue(cushions, day(10, d), early), "Oct \(d)")
        }
    ...
    func testUnwindowedChoreIsUnchanged() {
        for d in 14 ... 20 {
            XCTAssertEqual(overdue(laundry, day(9, d)), 0)
        }
