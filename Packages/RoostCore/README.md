...
- **Models** — `Chore`, `Cadence`, `ChoreCategory`, `Person` (anne, wes), `Completion`, `Season`. `Chore` carries optional `season`, `together`, `weekdays`, and `dueDay` (all default when absent) and decodes straight from `data/chores.json`; `ChoreList.load(from:)` reads the file. `Chore.hasWindow` is true when either window key is set.
