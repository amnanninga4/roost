# Roost — what's going on

_Generated Tuesday September 15, 2026 at 2:46 PM CDT. Regenerate with `scripts/status-report.sh`._

## Running right now

| | |
|---|---|
| Server build | `f5aff0e` |
| Latest code on main | `657ac1d` |
| Chores loaded | 41, list version 5 |
| Phones paired | 2 |
| Push notifications | no key |
| Last morning digest | 2026-09-15 |
| Last backup | ok — 2026-09-15 09:37 UTC |

## Waiting on you

Only things Fable cannot decide or do. Carrying the `needs-wes` label is what puts
something here — if it is not labelled, it is not yours, and it should not be on your plate.

- **Appearance choice in Settings: System, Light, Dark**  [#65](https://github.com/amnanninga4/roost/pull/65)

Things only you can do, that are not code, live in the *Blocked on Wes* table of
`OPEN-ITEMS.md`. Today that is the APNs key, Anne's phone, and a Tailscale key.

## Fable is handling these

Here so you can see them, not so you can do them. No action needed.

- A status report Wes can actually read  [#92](https://github.com/amnanninga4/roost/pull/92)

## Waiting on Anne

Three questions have been open on the kickoff thread since 09-13, re-asked twice.
The chore list is not final until they land:

1. Garbage split, third location — basement/bathroom/kitchen, or basement/bedroom/bathroom?
2. Which months does mowing run? April through October was proposed.
3. "Trim Wes's hair" is written as every two months, pinned to Anne. Right person, right rhythm?

Thread: **Apple Dev 3.0 ↔ Anne Claude — roost kickoff**  [#1](https://github.com/amnanninga4/roost/issues/1)

## The phones

- iPhone                     00008140-000A48800AEB001C (UDID)              connected   iPhone 16 (iPhone17,3)       physical 

## Open items, counted

- blocked on Wes: 6
- owned by Fable: 4

Full text and the reasons: `OPEN-ITEMS.md` in the repo.
