...
- **Early windows.** A `dueDay` of 1–6 gives a window that opens in the month before its period. For a windowed chore the period a date or a completion belongs to is its calendar period, or the next one once that next period's window has opened (`Scheduler.effectivePeriod(for:containing:)`), so "Clean under cushions" is due Sep 29 – Oct 5 as October's chore, a completion on Sep 30 is October's, and a late completion after a window still clears the period it was late for. One known limitation: `HandoffRules` keeps counting calendar periods, so a handoff offered during the early days of such a window is refused until the 1st.
...
- Clean under cushions again: on Sep 29 the item reports October's period (index 9); a completion on Sep 30 leaves nothing due Oct 1–29 and the November window opens Oct 30 (late Nov 6). Change litter completed late on Sep 27 clears September; October opens Oct 19.
