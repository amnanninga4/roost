...
Roost/
  ...
  Sources/
    ...
    Screens/SettingsScreen.swift Gear -> Settings: paired as, device, paired since, appearance, server, last sync, Unpair, version
  ...
  docs/settings.png           simulator screenshot, Settings on a light phone with Appearance on System
  docs/settings-dark.png      the same phone, still set to light, with Appearance on Dark — the override is the whole point of the pair
...
- **Appearance** — System, Light, or Dark, as a segmented picker. The choice applies to the whole app the
  moment it is made, and it is kept on this phone.
...
**Appearance** is one key in `UserDefaults` (`Appearance`, in `Models/Appearance.swift`) read by `RoostApp` as a
single `.preferredColorScheme` on the window's content. That is why one tap in the picker repaints the sheet it
was tapped in, the Tasks tab behind it, and Kitchen mode's full-screen cover together — they are all inside that
window. `system` maps to `nil`, which is "do not override", so it is the phone's setting again rather than a
third palette. It is per device and never synced: two phones, two people, two ideas about a screen at midnight.
A stored value this build does not recognise reads as `system` rather than leaving the picker with no selection.
**The widget cannot follow it.** A widget is a separate process WidgetKit renders in the system appearance, and
nothing the app writes reaches it — so the Home Screen stays on the phone's setting, which the footer under the
picker says out loud. At the accessibility text sizes the segmented control gives way to the same `Picker` drawn
as a menu row, because three words cannot share one row's width once each is 50-odd points tall.

`SettingsScreen` repeats the same `.preferredColorScheme` on itself, and that line is load-bearing: a sheet
presented while the window already carries an override keeps the scheme it was born with. Without it, picking
Light with Dark already on repainted the Tasks screen behind and left the sheet dark until it was dismissed —
which is exactly the moment the reader is looking at the sheet.
