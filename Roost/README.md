...
Roost/
  ...
  Sources/
    ...
    Screens/ListsScreen.swift  the Lists tab: ListPage, the segmented control, the paged TabView that hosts the four pages and applies the list chrome once
    Screens/WishlistScreen.swift  Wishlist page: composer with a price field, price chips, header total, Bought section, undo
  ...
  docs/lists.png              simulator screenshot, Lists tab on Shopping; docs/lists-dark.png in dark mode
...
| `paired` | paired as Anne: ten daily chores, the whole escalation ladder, one of each handoff state, four shopping rows, two wishlist rows, four meals, two projects (one project due in three days with a step that is Wes's) |
...
The root is `RootTabView`, a `TabView` over the `RootTab` enum: Tasks (`checklist`), Lists (`list.bullet.rectangle`), More (`ellipsis.circle`), tinted `RoostColor.accent`. The Lists tab is `ListsScreen`: a segmented control (symbols at accessibility sizes) over a paged `TabView` bound to the same `AppStorage` selection, so a swipe and a tap do the same thing and every page keeps its state; the list chrome is applied once there. The More tab holds Household (Kitchen mode full screen, All chores pushed) and This phone (Settings pushed, Sync now with the status line), with the app version in the footer.
...
Header: "N items · $X total" over the open items (the total is omitted when nothing has a price). "Add something you'd like…" takes a title and, once there is one, a price field ("599", "12.50"); a price that does not parse still saves the title without one. Rows show the check circle, the title, a price chip when present, and the adder's avatar. Ticking moves a row into Bought, exactly like Shopping; Clear bought and swipe-delete share Shopping's undo.
