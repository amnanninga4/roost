...
The list pages are paired as Anne against a local server, each in light and dark (recaptured inside the Lists tab). Shopping has four rows still to buy over a Bought section with "Clear bought", and the header line reads "Synced just now" because the pass had only landed a moment before. Meals has one idea marked NEXT UP and three with a tag and a "last made" line, one of them a weekday and one a date. Projects has a finished card open (DONE chip, full bar, Archive) over a second card at 2/4 with a due label and an owned step.
...
Roost/
  ...
  Sources/
    ...
    Screens/RoostSegmentedControl.swift  the Lists page picker: sliding pill on matchedGeometryEffect
    Screens/WishlistScreen.swift  Wishlist page: composer with a price field, price chips, header total, Bought section, undo, Edit sheet from the row menu
    Screens/MoreScreen.swift  the More tab: Kitchen mode, All chores, Settings (pushed), Sync now + status, version
    ...
    Models/Records.swift      SwiftData models: ChoreRecord (season, together, weekdays, dueDay, rotation, missPenalty), CompletionRecord, SyncState (and the schema list)
    ...
    Models/ListRecords.swift  SwiftData models for the lists: ShoppingItemRecord, WishlistItemRecord, MealRecord, ProjectRecord, SubtaskRecord; the ListRecord sync bookkeeping + PatchFields
    ...
    Models/ListPresentation.swift pure: the list screens' arithmetic — the bought/to-buy split, project progress, the sortOrder plan for a drag, the "last made" wording, ProjectDates
    ...
    Models/WindowCopy.swift   pure: a chore's due window -> "Fri–Sat", "Sun", "by the 25th"
    ...
    Notifications/NotificationPlanner.swift       pure: one day's due list -> ids, copy, Chicago fire times; project due-day morning reminder
    ...
    Screens/TodayScreen.swift Tasks tab: the store queries, the plan, check-off and un-check, the celebration counter
    ...
    Screens/KitchenScreen.swift Kitchen mode: full-screen counter view of both people, screen stays on; More → Kitchen mode
    Screens/StreakHeaderView.swift the head-to-head block from the mockup
    Screens/ShoppingScreen.swift Shopping page: composer, to-buy rows, Bought section + "Clear bought", check-off, swipe to delete with undo, Edit sheet from the row menu
    ...
    Screens/ListParts.swift   pieces the four list pages share: header + sync line, the composer, ListEditSheet, check circle, avatar, chips, the "Didn't sync" marker, empty states, swipe-to-delete, the undo bar and its five-second window, and `RoostTextActionStyle` — a text-only action that holds 44 pt, used from the onboarding screens too
    ...
    Screens/ChoreListScreen.swift the R-2 list, reachable from More as "All chores"
  ...
  Resources/Assets.xcassets   the asset catalog; AppIcon.appiconset/AppIcon.png is the only thing in it
  Shared/                     RoostCopy.swift (daysLate), snapshot shape + store — compiled into the app and the widget (`RoostSnapshot`, `SnapshotStore`, `RoostCopy.daysLate`), and nothing else crosses between them
  ...
  Widget/                     the WidgetKit extension (bundle id xyz.hinescreative.roost.widget)
    ...
    WidgetStrings.swift       every user-facing string in the widget; lateness uses RoostCopy.daysLate like the app
  ...
  Tests/                      in-memory ModelContainer + URLProtocol stub; no network
    ChoreRowMetaTests.swift   the two-slot meta ordering
  UITests/                    XCUITest: one accessibility audit per screen, and the Shopping list's scroll metrics
    TodayBehaviourTests.swift the Today row menu, swipe, and streak line expand/collapse
  ...
  docs/meals.png              simulator screenshot, Meals page inside Lists; docs/meals-dark.png in dark mode
  docs/projects.png           simulator screenshot, Projects page inside Lists; docs/projects-dark.png in dark mode
  ...
  docs/app-icon.png           simulator screenshot, the Home Screen, for looking at the icon under the mask
```
...
The app icon is `Resources/Assets.xcassets/AppIcon.appiconset/AppIcon.png`: one 1024x1024 image with no alpha, which iOS scales down for every other size and masks the corners of itself. `swift scripts/make-app-icon.swift` draws it and overwrites it in place. It is a placeholder — a white perch on the accent green — added because App Store Connect rejects an upload from a build with no icon; the real mark is D-7, Anne and Wes's to decide, and drops into the same file. The widget has no icon of its own: WidgetKit shows the app's. `docs/app-icon.png` is what it looks like on the Home Screen, where the corner mask is applied.
...
1. Add a test to `UITests/AccessibilityAuditTests.swift`. It launches a fixture, gets to the screen, and
   ...
   func testMyScreenAudit() throws {
       ...
       waitForTasks(in: app)
       openFromMore("My screen", in: app)
       XCTAssertTrue(app.staticTexts["SOMETHING ON IT"].waitForExistence(timeout: Self.timeout))
       try audit(app)
   }
   ```

   `openTab(_:in:)`, `openList(_:in:)`, and `openFromMore(_:in:)` are in `RoostUITestCase`. Wait for something that is
...
It runs on launch, when the app returns to the foreground, after every check-off and every list write, and from More → Sync now. An in-flight guard coalesces overlapping calls into at most one extra pass.
...
**The status line.** `SyncCoordinator.statusLine` is the one line under every tab header: "Syncing…", "Not synced yet", "Not paired · More → Settings", "Offline · will retry (<reason>)", or how long ago the last pass landed. The wording is in `Strings.Sync`; the bands are `SyncStatusCopy`. A pass that has only just finished reads "Synced just now" — under five seconds there is nothing to count, and `RelativeDateTimeFormatter` answered "in 0 sec." there, which reads as a duration rather than a moment. After that it is "Synced 28 sec. ago", then minutes, and once an hour has passed the clock time ("Synced at 8:41 AM") says more than a count.

## Screens

The root is `RootTabView`, a `TabView` over the `RootTab` enum: Tasks (`checklist`), Lists (`list.bullet.rectangle`), More (`ellipsis.circle`), tinted `RoostColor.accent`. The Lists tab is `ListsScreen` with a `RoostSegmentedControl` (one segment per page, the selection a pill sliding on `matchedGeometryEffect` with the `quick` spring; symbols only at accessibility sizes, each still named for VoiceOver) over the paged `TabView` bound to the same `AppStorage` selection, so a swipe and a tap do the same thing and every page keeps its state; the list chrome is applied once there. The More tab holds Household (Kitchen mode full screen, All chores pushed) and This phone (Settings pushed, Sync now with the status line), with the app version in the footer.
...
`TodayPlanner.plan` feeds `RoostCore.Scheduler` the active chores, the non-removed completions, and the live handoffs, with `activeFrom` = the household start the server sent (see [The household start date](#the-household-start-date)). For each person it lists what is due, cat care first, most overdue first, followed by rows completed today so they can be un-checked. The header shows the Chicago date, the streak block, the week's tallies, and the sync status line. The paired person's own column comes first (`TodayBoard.columnPeople`); an unpaired phone keeps Anne then Wes. The balancer stays nil: turning it on is a separate, deliberate change (`Packages/RoostCore/README.md`).

A together chore is a row in both columns with a TOGETHER chip where a pinned name goes; either check-off clears both rows, both tallies move, and it cannot be offered. A chore with a season (Mow lawn, April to October) is simply absent outside it. A chore with a due window (`weekdays` or `dueDay`) is only owed inside that window; never-owed and not-yet follow RoostCore, and overdue counts from the window end. A chore may state its own `rotation` (and an optional `missPenalty`); the app carries both through the record and `/sync` but shows no chrome for them in this lane.

**Streak header.** Collapsed by default on every launch (and persisted at `@AppStorage("roost.today.streakExpanded")`): one `monoTally` line under the title — "You 6 — 4 Anne · 12-day streak ›", each score in its person's colour, the streak being the longer of the two (`StreakHeaderModel.scoreLine(me:)` picks the halves and drops the streak clause at zero). A tap expands the familiar card on the `standard` spring. Expanded, `StreakHeaderModel` takes the plan's per-person streaks (`RoostCore.Tallies.streak`, so `activeFrom` comes from `SyncState` and a given-away daily counts for whoever took that day) and weekly tallies. Each side shows the name, the streak number, and "day streak". The side with the strictly higher week tally (the same number the week bar uses) gets a "CURRENTLY AHEAD" tag, its card in the `accentSoft` role behind an `accent` hairline, and its number in the `bonus` role; a tie tags nobody, and the pill is laid out invisibly on the other card so taking the lead does not move anything. Under the block a mono line carries the week's completions: `Anne · 14   Wes · 11`.

**Escalation.** The stage is a 3-pt rounded bar along the row's leading edge in the stage colour plus the tinted days-late chip; the full-row wash is gone. `EscalationStage` picks a `RoostColor.Role` (`TaskStyle.swift`): due today = `textPrimary`, 1–2 days = `nudge`, 3–4 = `warning`, 5+ = `danger`, each overdue stage's soft partner filling the days-late chip. The meta line is capped at two elements by urgency (`ChoreRowMeta`): days-late, then an active handoff note, then the together/giver/name chip — "from X" is the giver's chip in the giver's colour, "On the other phone" is gone. The card runs loudest first (`TodayBoard.ordered`): alert, then pointed, then nudge, then what is merely due today, with the planner's own order (cat care first, then most days late) deciding inside one stage — the same order Kitchen mode uses, so the two screens agree. `EscalationCopy.subtitle` adds a line under the title once a row is overdue: nudge → "Still no <title>…" (title lowercased to read mid-sentence, unless it starts with an acronym like "PM wet cat food"), pointed → "The cat has feelings about this." for cat-care rows and "Getting overdue." for home rows, alert → "<Title> emergency". Due-today rows and done rows show no subtitle.

**Check-off.** Checking a row inserts a `CompletionRecord` (UUID id, `completedAt` now, UTC) and kicks a sync; tapping a done row soft-deletes it. The whole row is the button, and the check control holds `RoostSpacing.minTapTarget` (44 pt) on its own inside it. The press is `.roostPressQuiet` everywhere (the tap already carries a haptic). The circle becomes a filled checkmark through `.contentTransition(.symbolEffect(.replace))` and beats 1.0 → 1.15 → 1.0 on the `quick` spring (skipped under Reduce Motion); the row itself moves on `RoostTransition.checkOff`; the "N DUE" count and the week's tallies roll with `.contentTransition(.numericText())` on `RoostMotion.standard`; and `RoostHaptic.checkOff` / `.undo` fire on the tap through the `.roostHaptic(_:trigger:)` counters, so a cold launch or a delta arriving from the other phone never buzzes. Every animation goes through `RoostMotion.reduceMotionAware`, which under Reduce Motion is `nil` — no animation, not a faster one.

**Row chrome.** Every row's long-press menu lifts a preview card with the full metadata over Hand off / Withdraw / Show in All chores (the last routes through `RootNavigation` to More → All chores); a trailing swipe on an offerable row reveals Hand off in the assign colour (hand-built — the board is a ScrollView, where `.swipeActions` does not exist).

**States.** `TodayBoard.notice` produces the one line under the header, and in every ordinary state ("Syncing…", "Synced just now", "Synced 5 min. ago", "Not synced yet") it is `SyncCoordinator.statusLine` verbatim — the same string the list pages print in their own headers, so two tabs of the same app can never disagree about when the last pass landed. It overrides that line in exactly two states: unpaired reads "Not paired yet · More → Settings", because the line has to say where to go; and a failed pass reads "Synced 5 min. ago · offline, will retry" in the `notice` role, because that is the one moment the time of the last good pass matters and "will retry" alone does not carry it. Its leading clause is `SyncStatusCopy`'s own phrase, so this screen never words "when" differently from the rest of the app, and a test pins that. Offline is never a modal. A person with no chores on the day reads "Nothing due today"; a person who has checked off everything they owed reads "Nothing left" above their done rows, which stay there to be un-checked. Pull down anywhere on the screen to run `SyncCoordinator.syncNow()`.
...
**Dynamic Type.** The screen is built for `accessibility5`, not merely survivable at it. `AnyLayout` swaps four rows into stacks at accessibility sizes: the streak cards, the person header (name / YOU / count), the week tally, and a row's meta line. The "CURRENTLY AHEAD" pill is laid out invisibly at normal sizes so taking the lead does not shove the cards, and dropped entirely when stacked, where an invisible pill would be a blank line of 40 pt type. The category badge — decoration, already `accessibilityHidden` — drops out at accessibility sizes rather than costing the title a third of its column. No title truncates at any size. Rows stagger in 40 ms apart on first appearance per launch only (`RoostMotion.staggerDelay`).

**All chores** (More → All chores) got the same treatment: it no longer carries its own `NavigationStack` (it is pushed onto the Tasks tab's), the cadence groups are inset cards on the page colour with a mono count each, grouped by six cadences (Daily … Every three months); a chore with a due window carries it under the title ("Fri–Sat", "Sun", "by the 25th"), a seasonal chore carries its months, and a chore pinned to one person wears their name (a together chore wears TOGETHER). The two window keys (`weekdays`, `dueDay`) ride `/sync` like `season`. At accessibility sizes its rows drop the category badge, move the pinned chip under the title, and take a `@ScaledMetric` vertical padding, because a fixed 8-pt gap between two 40-pt rows reads as one wrapped sentence.
...
**Offering.** Long-press a row in your own column that belongs to the current period: the menu offers "Ask Wes to take this" (the other person's name) under a preview card with the full metadata, and a trailing swipe reveals the same Hand off in the assign colour. One confirmation names the chore and its period — "Ask Wes to take Laundry this week?", the same period wording the server's push uses — and then a `HandoffRecord` is written locally, state `pending`, with a client-generated UUID, and the sync pass POSTs it. The row keeps its place and its meta line reads "Asked Wes · waiting".
...
More → Kitchen mode presents `KitchenScreen` full-screen (`fullScreenCover`, no navigation chrome) for a phone propped on the counter. It shows both people at once and is read-only; check-off stays on the Tasks tab. Close is the only way to leave — the board itself is not a tap target.
...
Header: "N items · $X total" over the open items (the total is omitted when nothing has a price). "Add something you'd like…" takes a title and, once there is one, a price field ("599", "12.50"); a price that does not parse still saves the title without one. Rows show the check circle, the title, a price chip when present, and the adder's avatar. A row's long-press menu offers Edit (a medium-detent sheet reusing the composer styling — the title plus the price; saving goes through `ListActions.renameWishlistItem` and the usual PATCH replay) and Delete (the same five-second undo as the swipe). Ticking moves a row into Bought, exactly like Shopping; Clear bought and swipe-delete share Shopping's undo.

Empty, the screen reads "Nothing on the wishlist." over "Add the things you'd buy some day."
...
- **09:00 Chicago** one notification per unfinished project due that day ("Garage trash is due today"), via `NotificationPlanner.planProjects`.
