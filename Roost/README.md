...
**States.** `TodayBoard.notice` produces the one line under the header, and in every ordinary state ("Syncing…", "Synced 28 sec. ago", "Not synced yet") it is `SyncCoordinator.statusLine` verbatim — the same string the three list tabs print in their own headers, so two tabs of the same app can never disagree about when the last pass landed. It overrides that line in exactly two states: unpaired reads "Not paired yet · gear menu → Settings", because the line has to say where to go; and a failed pass reads "Offline · last synced 5 min. ago" in the `notice` role, because that is the one moment the time of the last good pass matters. The time it formats there uses the same `RelativeDateTimeFormatter` style the coordinator uses, and a test pins that. Offline is never a modal. A person with no chores on the day reads "Nothing due today"; a person who has checked off everything they owed reads "Nothing left" above their done rows, which stay there to be un-checked. Pull down anywhere on the screen to run `SyncCoordinator.syncNow()`.
