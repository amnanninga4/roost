...
Roost/
  ...
  Sources/
    ...
    Models/Records.swift      SwiftData models: ChoreRecord (season, together), CompletionRecord, SyncState (and the schema list)
...
A together chore is a row in both columns with a TOGETHER chip where a pinned name goes; either check-off clears both rows, both tallies move, and it cannot be offered. A chore with a season (Mow lawn, April to October) is simply absent outside it.
