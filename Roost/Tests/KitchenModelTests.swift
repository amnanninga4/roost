...
final class KitchenModelTests: XCTestCase {
    ...
    private let chores: [Chore] = [
        ...
        Chore(id: "refill-cat-water", title: "Refill cat water", cadence: .daily, fixedAssignee: .wes, category: .catCare),
    ...
    private func seed(_ chores: [Chore], completions: [(id: String, choreId: String, person: Person, day: Int)]) throws {
    ...
    private func model(asOf date: Date) throws -> KitchenModel {
        let chores = try context.fetch(FetchDescriptor<ChoreRecord>(predicate: #Predicate { !$0.retired }, sortBy: [SortDescriptor(\.sortOrder)]))
        ...
        return KitchenModel(chores: chores, completions: completions, activeFrom: state?.activeFrom, asOf: date, calendar: cal)
    }
    ...
    func testGroupsOverdueByPersonSortedByStageDescending() throws {
        ...
        XCTAssertEqual(anne.overdue.map(\.copy), ["Vacuum basement emergency", "Getting overdue.", "Still no scoop litter…"])
        ...
        XCTAssertFalse(wes.overdue.contains { $0.chore.id == "laundry" } || anne.overdue.contains { $0.chore.id == "laundry" },
    ...
    func testAlertStageItemsFromBothPeopleAppearInTheBanner() throws {
        ...
        XCTAssertEqual(m.alerts.map(\.chore.id), ["vacuum-basement", "refill-cat-water"], "both 5 days late; Anne first on a tie")
        ...
        XCTAssertFalse(m.alerts.contains { $0.chore.id == "wipe-tables" }, "pointed stays in the column, out of the banner")
    ...
    func testMostOverdueAlertLeadsTheBanner() throws {
        // Wes's cat water never done since Sep 1 → 7 days late on Sep 8; Anne's vacuum done Sep 2 → Sep 3 missed → 5 days late.
        ...
        XCTAssertEqual(m.alerts.map(\.chore.id), ["refill-cat-water", "vacuum-basement"], "7 days late leads 5 days late even though Anne's column comes first")
    ...
    func testEmptyPlanIsCaughtUp() {
        ...
        let m = KitchenModel(plan: TodayPlanner.plan(chores: [], completions: [], asOf: day, activeFrom: cal.startOfDay(day), calendar: cal))
        ...
        XCTAssertTrue(m.columns.allSatisfy { $0.overdue.isEmpty })
    ...
    func testFiveDayOverdueLitterBoxForWesIsAnEmergencyInTheBanner() throws {
        try seed([Chore(id: "scoop-litter", title: "Scoop litter", cadence: .daily, fixedAssignee: .wes, category: .catCare)], completions: [])
        let m = try model(asOf: sep6)
